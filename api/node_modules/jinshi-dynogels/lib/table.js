...
internals.createItem = (table, item, options, callback) => {
  ...
  const start = callback => {
    ...
    const paramNames =  _.isString(self.schema.updatedAt) ? self.schema.updatedAt : 'updatedAt';

    if (self.schema.timestamps && self.schema.createdAt !== false && !_.has(data, paramName)) {
      data[paramName] = new Date().toISOString();
    }

    if (self.schema.timestamps && self.schema.updatedAt !== false && !_.has(data, paramNames)) {
      data[paramNames] = new Date().toISOString();
    }
