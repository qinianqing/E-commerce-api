...
// 核心是优化写的性能
// 信息直接通过task任务从js_goods表来同步
// 只改变内部封装，外部所有接口不变，不影响外部调用
                ...
                if (n === spus.length){
                    ...
                    for (let k=0;k<spus.length;k++){
                        for (let m=0;m<results.length;m++){
                            if (results[m].attrs.goods_id === spus[k]){
                                items.push(results[m]);
                                break;
                            }
                        }
                    }
