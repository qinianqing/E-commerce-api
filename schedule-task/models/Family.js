...
const FamilyM = dynogels.define('js_family',{
    ...
    indexes:[
        { hashKey: 'vip', rangeKey: 'vip_expiredAt', type: 'global', name: 'VipStatusIndex' },
    ]
});

// if (env === 'dev' || env === 'build') {
//     dynogels.createTables({'js_family': { readCapacity: 5, writeCapacity: 5 }},(err) => {
//         if (err) {
...
class Family {
    ...
    getAllFanilyMsg(familys,callback){
        FamilyM.getItems(familys,(err,data)=>{
            callback(err,data)
        })
    }
    ...
    // 增加家庭账户余额
    updateBalanceByCashBack(amount,callback){
        FamilyM.update({
            user_id:this.user_id,
            family_id:this.family_id,
            balance:{$add:amount}
    ...
    // 按状态获取家庭
    getFamilyByVipStatusExpired(status,callback,last_key){
        if(last_key){
            FamilyM.query(status)
                .where('vip_expiredAt').between(0,Date.now()+1)
                .startKey(last_key)
                .limit(500)
                .usingIndex('VipStatusIndex')
                .exec((err,data)=>{
                    callback(err,data)
                })
        }else {
