...
const ScheduleM = dynogels.define('js_schedule',{
    ...
    // 建立索引防止出现数据取的不正确的问题
    indexes:[
        { hashKey: 'method', rangeKey: 'occur', type: 'global', name: 'OccurIndex' },
    ]
});

// if (env === 'dev' || env === 'build') {
//     dynogels.createTables({'js_schedule': { readCapacity: 5, writeCapacity: 5 }},(err) => {
//         if (err) {
...
class Schedule {
    ...
    // 传入的时间数据都是iso格式的字符串
    getSchedules(p,last_key,callback){
        if (last_key){
            ScheduleM.query(this.method)
                .where('occur').lte(p)
                .filter('status').equals(0)
                .startKey(last_key)
                .usingIndex('OccurIndex')
                .limit(1000)
                .exec((err,data)=>{
                    callback(err,data)
                })
    ...
    // 直接获取某occur时间的所有未处理数据
    getSchedulesByOccur(p,last_key,callback){
        if(last_key){
            ScheduleM.query(this.method)
                .where('occur').equals(p)
                .filter('status').equals(0)
                .usingIndex('OccurIndex')
                .startKey(last_key)
                .limit(1000)
