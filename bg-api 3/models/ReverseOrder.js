...
const ReverseOrderM = dynogels.define('js_reverse_order',{
    ...
    schema:{
        ...
        /*
            ...
            */
        status:Joi.string(),// _INIT(初始状态)\_DENY(拒绝)\_SENDBACK(同意)\*SUCCESS(完成)\*CANCEL(取消申请),暂时没有的状态:PROCESS_
...
class ReverseOrder {
    ...
    updateReverseOrderForExpress(msg,callback){
        ReverseOrderM.update({
            user_id:this.user_id,
            reverse_id:this.reverse_id,
            status:this.status,
    ...
    // 获取订单列表，按创建时间倒叙排列
    getAllReverseOrderList(callback,lastKey){
        if (lastKey){
            ReverseOrderM.scan()
            //.descending()
                .startKey(lastKey)
